  ...
  return (
    ...
    <form onSubmit={handleSubmit}>
      ...
      <FormField label="Name">
        <input className={inputClassName} id="worker_name" name="name" type="text" value={form.name || ""} onChange={(event) => updateField("name", event.currentTarget.value)} />
      </FormField>

      <FormField label="Protocol">
        <select className={inputClassName} id="worker_protocol" name="protocol" value={form.protocol || "localhost"} onChange={(event) => updateField("protocol", event.currentTarget.value)}>
      ...
      <FormField label="Host Name">
        <input className={inputClassName} id="worker_hostname" name="hostname" type="text" value={form.hostname || ""} onChange={(event) => updateField("hostname", event.currentTarget.value)} />
      </FormField>

      <FormField label="User Name">
        <input className={inputClassName} id="worker_username" name="username" type="text" value={form.username || ""} onChange={(event) => updateField("username", event.currentTarget.value)} />
      </FormField>

      <FormField label="Password">
        <input className={inputClassName} id="worker_password" name="password" type="text" value={form.password || ""} onChange={(event) => updateField("password", event.currentTarget.value)} />
      </FormField>

      <FormField label="Base Path">
        <p className="mb-2 text-sm text-gray-500">Leave blank to use the default path {defaultBasePath}</p>
        <input className={inputClassName} id="worker_base_path" name="base_path" type="text" value={form.base_path || ""} onChange={(event) => updateField("base_path", event.currentTarget.value)} />
      </FormField>
      </div>
      ...
      <FormField label="Worker Capabilities">
        <p className="mb-2 text-sm text-gray-500">
          Values are separated by commas. <a href="/help_topics/workers">Help</a>
        </p>
        <textarea className={inputClassName} id="worker_capabilities" name="capabilities" rows={3} value={form.capabilities || ""} onChange={(event) => updateField("capabilities", event.currentTarget.value)} />
      </FormField>

      <FormField label="Maximum Builds">
        <p className="mb-2 text-sm text-gray-500">0 = unlimited</p>
        <input className={inputClassName} id="worker_max_builds" name="max_builds" type="text" value={form.max_builds ?? 0} onChange={(event) => updateField("max_builds", event.currentTarget.value)} />
      </FormField>
