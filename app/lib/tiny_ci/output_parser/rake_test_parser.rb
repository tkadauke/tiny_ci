module TinyCI
  module OutputParser
    class RakeTestParser < Base
      STATUSES = { "." => "success", "F" => "failure", "E" => "error" }.freeze
      ...
      def parse!
        ...
        while !empty? && peek.line !~ /\*\* (Execute|Invoke)/
          ...
          if line =~ /(test_[a-zA-Z0-9_]*)\((.*?)\):\s+([\.FE])/
            ...
            $1 == "Error" ? parse_error! : parse_failure!
          end
        end
      end
      ...
      def parse_error!
        ...
        return unless line =~ /(test_[a-zA-Z0-9_]*)\((.*?)\)/
        ...
        test_case&.error!(error_message, backtrace)
      end
      ...
      def parse_failure!
        ...
        test_case = @result.test_case(test_name, test_case_name)
        test_case&.error!(error_message.join(" "), backtrace)
      end
    end
  end
end
