...
# End-to-end smoke test: configure a real Project + Plan + Worker, kick off
...
class BuildLifecycleTest < ActiveSupport::TestCase
  setup do
    ...
    @build = @plan.builds.create!(status: "pending", worker: @worker)
  ...
  test "should persist runner output when an unexpected exception ends the build in error" do
    ...
    error_build = @plan.builds.create!(status: "pending", worker: @worker)
  ...
  test "should pass nil-valued env vars through without crashing the shell" do
    @plan.update!(steps: <<~RUBY)
      ...
      sh "echo nil-env-ok"
    RUBY
    nil_env_build = @plan.builds.create!(status: "pending", worker: @worker)
  ...
  test "build fails when shell step exits non-zero" do
    ...
    failing = @plan.builds.create!(status: "pending", worker: @worker)
  ...
  test "build with multiple steps runs them in order" do
    @plan.update!(steps: <<~RUBY)
      ...
      sh "echo third"
    RUBY
    multi = @plan.builds.create!(status: "pending", worker: @worker)
  ...
  test "build with cd block scopes the working directory" do
    ...
    RUBY
    cd_build = @plan.builds.create!(status: "pending", worker: @worker)
