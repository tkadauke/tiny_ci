...
class TinyCI::ConfigTest < ActiveSupport::TestCase
  setup    { TinyCI::Config.instance.reload! }
  teardown { TinyCI::Config.instance.reload! }
  ...
  test "should get option from database" do
    ConfigOption.expects(:find_by).with(user_id: nil, key: "hello").returns(stub(key: "hello", value: "universe".to_yaml))
    File.expects(:read).returns([{ "hello" => { "type" => "String", "default" => "world" } }].to_yaml)
  ...
  test "should set option" do
    ...
    option.expects(:update).with(value: "galaxy".to_yaml)
    ConfigOption.expects(:find_or_create_by).with(user_id: nil, key: "hello").returns(option)
    File.expects(:read).returns([{ "hello" => { "type" => "String", "default" => "world" } }].to_yaml)

    TinyCI::Config.instance.set("hello", "galaxy")
  end

  test "should set several options at once via update" do
    TinyCI::Config.instance.expects(:set).twice
    TinyCI::Config.instance.update("hello" => "galaxy", "foo" => "bar")
  end

  test "should type cast to integer when reading" do
    ConfigOption.expects(:find_by).returns(stub(key: "hello", value: "15".to_yaml))
  ...
  test "should type cast to integer when writing" do
    ...
    option.expects(:update).with(value: 42.to_yaml)
    ConfigOption.expects(:find_or_create_by).returns(option)
  ...
  test "should type cast to hash" do
    ConfigOption.expects(:find_by).returns(stub(key: "hello", value: { "foo" => "bar" }.to_yaml))
