...
class TinyCI::Notifier::GrowlNotifierTest < ActiveSupport::TestCase
  ...
  test "should deliver success" do
    ...
    connection = stub
    connection.expects(:notify).with(
      "TinyCI Notification",
      "Success",
      all_of(regexp_matches(/some_project/), regexp_matches(/some_plan/), regexp_matches(/7/))
    )
  ...
  test "should deliver failure" do
    ...
    connection.expects(:notify).with(
      "TinyCI Notification",
      "Failure",
      all_of(regexp_matches(/some_project/), regexp_matches(/some_plan/), regexp_matches(/7/), regexp_matches(/failed/))
    )
    notifier.stubs(:connection).returns(connection)
